...
public class ArticleViewRankRepairTask {
    ...
    public void repairRankIndex() {
        try {
            ...
            List<BlogArticle> articles = blogArticleDAO.selectList(queryWrapper);
            ...
            for (BlogArticle article : articles) {
                ...
                articleIndexRedisService.promoteRankScore(
                        article.getId(),
                        article.getViewCount() == null ? 0L : article.getViewCount()
                );
            }

            Set<Integer> staleIds = new HashSet<>(articleIndexRedisService.getAllRankArticleIds());
            staleIds.removeAll(validIds);
            articleIndexRedisService.removeRankMembers(staleIds);
